...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/26 14:06:25 by hialpagu          #+#    #+#             */
/*   Updated: 2025/03/26 14:06:25 by hialpagu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	b_set(t_fractal *fractal)
{
	int		i;
	double	tmp;
	...
	while (i < fractal->iteration)
	{
		tmp = (fractal->zx * fractal->zx) - (fractal->zy * fractal->zy)
			+ fractal->cx;
		fractal->zy = 2.0 * abs_v(fractal->zy) * abs_v(fractal->zx)
			+ fractal->cy;
		fractal->zx = tmp;
		if (fractal->zx * fractal->zx + fractal->zy
			* fractal->zy >= __DBL_MAX__)
